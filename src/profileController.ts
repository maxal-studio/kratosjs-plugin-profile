...
import { KratosRequest, KratosReply, t } from "@maxal_studio/kratosjs";
...
export async function viewProfile(
  ...
  try {
    ...
    res.status(500).json({
      error: error.message || t("profile:view_profile.failed_to_view_profile"),
    });
  }
}
...
export async function updateProfile(
  ...
  try {
    ...
    if (!formatMediaKey) {
      res
        .status(500)
        .json({ error: t("profile:update_profile.media_format_unavailable") });
      return;
    }
    ...
    if (!user) {
      res
        .status(404)
        .json({ error: t("profile:update_profile.user_not_found") });
      return;
    }
    ...
    res.status(500).json({
      error:
        error.message || t("profile:update_profile.failed_to_update_profile"),
    });
  }
}
...
export async function changePassword(
  ...
  try {
    ...
    if (!newPassword || !confirmPassword) {
      res
        .status(400)
        .json({ error: t("profile:change_password.all_fields_required") });
      return;
    }
    if (newPassword !== confirmPassword) {
      res
        .status(400)
        .json({ error: t("profile:change_password.password_mismatch") });
      return;
    }
    if (newPassword.length < 8) {
      res
        .status(400)
        .json({ error: t("profile:change_password.password_too_short") });
      return;
    }
    ...
    res.json({
      success: true,
      message: t("profile:change_password.password_updated_successfully"),
    });
  } catch (error: any) {
    console.error("Error changing password:", error);
    res.status(500).json({
      error:
        error.message || t("profile:change_password.failed_to_change_password"),
    });
  }
}
